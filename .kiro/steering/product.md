...
- The default local browser path is the single-origin proxy at `http://localhost:8090`. Anonymous browsing, playback, and chat read should work there without any first-party account; authenticated Twitch chat sending is optional and uses the localhost-only device-code / token import flow.
- The channel workspace is not just a player shell. It includes playback controls, separate requested vs loaded quality state, diagnostics, LSF, and emote management. When backend renditions are known, the request menu should reflect actual requestable variants rather than only static presets. The lower workspace also supports an expandable panel with `Comfort` and `Dense` density modes.
