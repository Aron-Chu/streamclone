...
$Script:StreamcloneBootstrapOverlayPaths = @(
    ...
    'scripts/ensure-setup-control.sh',
    'scripts/streamclone-url-handler.sh',
    'scripts/register-setup-control-protocol.ps1',
    'scripts/register-setup-control-protocol.sh',
    'scripts/unregister-setup-control-protocol.ps1',
...
function Invoke-StreamcloneUpgrade {
    param(
        ...
        [ValidateSet('interactive', 'capture', 'summary', 'friendly')][string]$PullOutputMode = 'friendly'
    )
    if ([string]::IsNullOrWhiteSpace($Root)) {
        $Root = Get-EnvRepoRoot
    }
