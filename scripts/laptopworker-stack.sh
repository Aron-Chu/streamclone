...
case "$cmd" in
  ...
  setup-verify)
    ...
    echo "==> sudoers"
    if sudo -n test -r /etc/sudoers.d/streamclone-laptopworker 2>/dev/null && \
       sudo -n grep -q '/usr/local/sbin/streamclone-laptopworker-firewall' /etc/sudoers.d/streamclone-laptopworker 2>/dev/null; then
