...
import type { SourceStatus, StreamStat } from '../../api'
import { buildVodDeepLink } from '../../utils/vodLink.ts'
  ...
  if (!rows?.length) {
    ...
    return (
      <EmptyPanel
        ...
        detail={historySource ? historySource.message || 'Stream rows load from Twitch Helix when available.' : 'Stream history appears when Twitch Helix returns past broadcasts.'}
      />
    )
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="overflow-x-auto rounded border border-white/10 bg-white/[0.035]">
        <div className="grid min-w-[920px] grid-cols-[minmax(300px,1.45fr)_88px_88px_88px_88px_minmax(200px,1fr)] gap-3 border-b border-white/10 px-3 py-2 text-[11px] font-black uppercase text-zinc-500">
          <span>Stream</span>
          <span>Duration</span>
          <span>Average</span>
          <span>Peak</span>
        ...
        {rows.map(row => {
          const thumb = vodThumbnailUrl(row.thumbnailUrl)
          return (
            <div
              key={row.id}
              className="grid min-w-[920px] grid-cols-[minmax(300px,1.45fr)_88px_88px_88px_88px_minmax(200px,1fr)] gap-3 border-b border-white/5 px-3 py-3 text-sm font-bold text-zinc-300 transition last:border-b-0 hover:bg-white/[0.05]"
            >
