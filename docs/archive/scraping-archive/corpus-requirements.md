...
Streamclone maintains **one shared global archive corpus** in Azure Blob Storage — VOD **intelligence** (catalog, identity, viewer charts, chat aggregates, emote metadata), not Twitch video files. legacy-rollback-host runs long-lived **analytics-workers** that continuously export artifacts; Postgres is a disposable hot cache.
...
```mermaid
flowchart TB
  subgraph bearhost [legacy-rollback-host]
