...
| [`docs/hosted-production-vps.md`](../hosted-production-vps.md) | Where Silver/Gold workers run today |
...
|-------|------------|--------------|--------------|-----------------|
| **Live Pulse** | Helix viewer sampler + IRC collector → minute rollups (`chat_source=live`) | **hosted-production-vps** production stack (`streamclone-production-analytics`, local Postgres) | Automatic for tracked roster | Global/channel activity graphs (live-only filter) |
| **Manual VOD import** | Extension/portal `PulseBackfillManager` → GQL chat for one VOD | **hosted-production-vps** `analytics` (hosted API) | User clicks “load missed moments” / backfill API | Per-session imported view; labeled `chat_source=gql` |
| **Autonomous corpus (Silver/Gold)** | Bronze index → Silver TT charts → Gold GQL chat backfill at scale | **hosted-production-vps** (`scraper` + `analytics-workers`, single worker) | Operator config (`CORPUS_WORKERS_ENABLED=1`) | Hub **aggregate-only** tier counts (`corpusPipeline.silver\|gold`); no per-stream errors on public API |

**Production cutover (2026-07-02):** Hosted API SoT is **hosted-production-vps** (`hosted-production-vps`) — API, Postgres, Redis, Caddy tunnel, and **one** corpus worker. BearHost (`legacy-rollback-host`) is rollback-only until soak passes. Legacy split topology (BearHost API + VPS tailnet worker/collector) is retired; stop obsolete `streamclone-collector` / `hosted-production-vps-corpus` stacks on VPS.

**Corpus re-enable (2026-06-30):** Autonomous Silver/Gold workers run on hosted-production-vps only. Manual VOD import uses the hosted analytics service on hosted-production-vps. See [`docs/live-first-pivot.md`](../live-first-pivot.md) § “Corpus re-enable”.
...
- **Second worker (2026-07-02):** BearHost may run a **silver-only** remote worker against hosted-production-vps SoT over Tailscale (`deploy/docker-compose.bearhost-corpus-remote-worker.yml`). VPS keeps a single Gold canary worker; do not add a second worker on VPS without metrics review.
...
`lease_owner` accepts worker identity strings (e.g. `hosted-production-vps-0`) without schema change.
...
- Env: `SCRAPER_ID`, `GQL_WORKER_ID` (default host-derived, e.g. `hosted-production-vps-0`).
