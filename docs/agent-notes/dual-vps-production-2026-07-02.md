...
```
                    ...
                    api.streampulse.stream
                              |
              +---------------+---------------+
              |   hosted-production-vps (SoT)     |
              |   hosted-production-vps            |
              +---------------+---------------+
...
|------|------------|----------------|---------------|----------------|--------|
| **hosted-production-vps** | Yes (`api.streampulse.stream`) | SoT | Yes | Canary (1 gold) + API plane | Yes |
...
Next env target on **hosted-production-vps** `analytics` service:
...
1. **IRC:** `PULSE_TOP500_ADMISSION_ENABLED=false` on hosted-production-vps; restart analytics.
2. **Workers:** `docker compose ... down` on BearHost worker project only.
3. **Public API:** BearHost rollback stack + DNS/tunnel flip is **operator-only** and **not** default — hosted-production-vps remains canonical unless explicitly approved.

## Related files

- `deploy/docker-compose.bearhost-worker.yml`
- `deploy/env/profile-bearhost-worker.env.example`
- `deploy/docker-compose.hosted-production-vps-production.yml`
- `docs/agent-notes/hosted-production-vps-production-migration-2026-07-02.md`
