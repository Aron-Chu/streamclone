...
**Goal:** Move production SoT from expiring BearHost (`legacy-rollback-host`) to bigger VPS
`hosted-production-vps` (`hosted-production-vps`). API + Postgres + Redis + **single** corpus worker on
...
```bash
bash scripts/hosted-production-vps-production-backup-bearhost.sh
```
...
## Phase 2 — Restore on hosted-production-vps

```bash
DUMP_PATH=runtime/backups/streamclone-bearhost-*.dump \
  bash scripts/hosted-production-vps-production-restore.sh
```
...
1. Copy `deploy/env/profile-hosted-production-vps-production.env.example` →
   `deploy/env/profile-hosted-production-vps-production.local.env` on VPS.
...
```bash
bash scripts/hosted-production-vps-production-deploy.sh
```
...
**Stop:** decommission `deploy/docker-compose.hosted-production-vps-corpus.yml` worker-only stack
...
1. Point Cloudflare tunnel `api.streampulse.stream` → `http://127.0.0.1:8090` on **hosted-production-vps**.
