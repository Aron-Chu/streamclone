...
4. For streampulse-vps SoT: restore BearHost `pg_dump -Fc` via `scripts/streampulse-vps-production-restore.sh` (requires `STREAMPULSE_PG_RESTORE_CONFIRM=YES_I_HAVE_PROD_BACKUP`; uses `streamclone-production` compose project), then migrate, then smoke.
