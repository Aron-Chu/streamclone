...
Cross-repo snapshot after R2 emote cutover and pre-launch hardening. **Backend corpus-auth deploy completed 2026-07-02** on hosted-production-vps; portal UI still needs Cloudflare Pages deploy.
...
VPS_HOST=root@hosted-production-vps \
...
## IRC admission restore (hosted-production-vps)
...
BearHost (`legacy-rollback-host`) runs **`deploy/docker-compose.bearhost-worker.yml`** against VPS SoT over Tailscale — no public tunnel. Start: `bash scripts/bearhost-worker.sh`.

## Safe limited admission (hosted-production-vps) — superseded
...
PULSE_PROBE_SSH_TARGET=root@hosted-production-vps \
...
3. `docker compose -f deploy/docker-compose.hosted-production-vps-production.yml up -d emote analytics pulse-caddy analytics-workers`
