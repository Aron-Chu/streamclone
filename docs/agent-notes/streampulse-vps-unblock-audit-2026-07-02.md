...
| Check | Result |
|-------|--------|
| SSH `root@hosted-production-vps` from Windows | **Permission denied** (no `~/.ssh/id_ed25519`; bearhost key rejected for root) |
...
1. **Restore compose project mismatch** — `hosted-production-vps-production-restore.sh` now uses `streamclone-production` project + same compose stack as deploy; resolves postgres by service id; requires `STREAMPULSE_PG_RESTORE_CONFIRM=YES_I_HAVE_PROD_BACKUP`; refuses if analytics/workers running.
...
3. **Duplicate worker risk** — deploy preflight detects old `hosted-production-vps-corpus` containers and exits with stop instructions (does not auto-down).
...
1. From WSL: confirm `ssh -i ~/.ssh/id_ed25519 root@hosted-production-vps`.
2. On VPS: if old corpus stack running → `docker compose -f deploy/docker-compose.hosted-production-vps-corpus.yml down`.
3. Copy `deploy/env/profile-hosted-production-vps-production.env.example` → `.local.env` + secrets on VPS.
4. `bash scripts/hosted-production-vps-production-backup-bearhost.sh`
5. `STREAMPULSE_PG_RESTORE_CONFIRM=YES_I_HAVE_PROD_BACKUP DUMP_PATH=... bash scripts/hosted-production-vps-production-restore.sh`
6. `bash scripts/hosted-production-vps-production-deploy.sh`
