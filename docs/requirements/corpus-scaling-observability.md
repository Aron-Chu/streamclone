...
|-------|------------|--------------|--------------|-----------------|
| **Live Pulse** | Helix viewer sampler + IRC collector → minute rollups (`chat_source=live`) | BearHost API + hosted-production-vps collector | Automatic for tracked roster | Global/channel activity graphs (live-only filter) |
...
| **Autonomous corpus (Silver/Gold)** | Bronze index → Silver TT charts → Gold GQL chat backfill at scale | **hosted-production-vps** (`scraper` + `analytics-workers`) | Operator config (`CORPUS_WORKERS_ENABLED=1`) | Hub **aggregate-only** tier counts (`corpusPipeline.silver\|gold`); no per-stream errors on public API |

**Corpus re-enable (2026-06-30):** Autonomous Silver/Gold workers run on hosted-production-vps only. BearHost API keeps `CORPUS_WORKERS_ENABLED=false`. Manual VOD import stays on BearHost. See [`docs/live-first-pivot.md`](../live-first-pivot.md) § “Corpus re-enable”.
...
- [ ] Gold job cannot mark `done` with unresolved segment failures (after PR 0B-3 or equivalent completion hook).
