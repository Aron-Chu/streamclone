...
**Last check-quick:** PASS (2026-06-20, WSL `make check-quick`)
...
- [ ] TASK-031 — BearHost corpus smoke (script ready; **operator run** on staging)
- [ ] TASK-031B — Restore drill smoke (script ready; **operator run** with `STREAM_ID`)
...
|------|------|--------|--------------|
| 2026-06-20 | TASK-031/031B | Scripts ready; live Azure bronze + restore not run in agent session | Operator: `CORPUS_WORKERS_ENABLED=1 bash scripts/bearhost-corpus-smoke.sh`; `STREAM_ID=… bash scripts/archive-restore-drill.sh` |
...
Database changes: none
Env/config changes: none
Acceptance criteria: `curl -s -o /dev/null -w '%{http_code}' http://141.11.243.103/v1/admin/archive/jobs` returns **401** (auth middleware) not **404** from metadata
...
Goal: Prevent archive admin token entry over raw HTTP on BearHost (`141.11.243.103`, no TLS).
...
Acceptance criteria: `http://141.11.243.103/admin/archive` shows CLI help, not token input; localhost shows gate
...
**Access:** SSH tunnel `ssh -L 3000:127.0.0.1:3000 streamclone@141.11.243.103`; **no** Caddy route to `/metrics`.
