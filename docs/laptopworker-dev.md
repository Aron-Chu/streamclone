...
| **BearHost VPS** | **No** — separate deploy scripts | `make bearhost-*`, `scripts/bearhost-pulse-redeploy-remote.sh`, etc. |
...
| Production / hosted API | `https://api.streampulse.stream` | Public ingress (streampulse-vps + Cloudflare Tunnel); BearHost rollback until soak |
...
Do **not** commit OAuth tokens or machine-specific URLs into the repo. For production API tests, use `https://api.streampulse.stream` in Options when you intend to hit the **hosted** API (streampulse-vps via Cloudflare, not BearHost rollback).
