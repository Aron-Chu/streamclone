...
When BearHost ran **Pulse API-only mode** (pre-VPS cutover), Silver/Gold jobs in Postgres were **paused**, not “stuck.” Production now runs workers on hosted-production-vps with local Postgres.
