...
Streamclone full production on BearHost VPS **141.11.243.103** — IP-only HTTP until a domain and ACME are configured.
...
SSH key: `~/.ssh/id_ed25519_bearhost_streamclone` → `streamclone@141.11.243.103`.
...
ssh -i ~/.ssh/id_ed25519_bearhost_streamclone streamclone@141.11.243.103
...
ssh -i ~/.ssh/id_ed25519_bearhost_streamclone root@141.11.243.103
...
APP_DOMAIN=141.11.243.103

PUBLIC_ORIGIN=http://141.11.243.103
...
4. `http://141.11.243.103/` frontend 200
...
# on your PC:
ssh -i ~/.ssh/id_ed25519_bearhost_streamclone -L 3001:127.0.0.1:3000 streamclone@141.11.243.103
