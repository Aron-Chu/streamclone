...
Production host: **http://legacy-rollback-host/** (legacy-rollback-host, HTTP until domain + HTTPS).

## Site

| What | URL |
|------|-----|
| Home / directory | http://legacy-rollback-host/ |
| Analytics | http://legacy-rollback-host/analytics |
| Admin archive (operator) | http://legacy-rollback-host/admin/archive |
...
| **legacy-rollback-host** | **No** — same UI, but token import is blocked off localhost by design |
...
## VOD / bronze collection status (legacy-rollback-host only)
...
**On the VPS directly** (after `ssh streamclone@legacy-rollback-host`):
...
Panels: archive jobs, backfill queue, coverage by tier, bronze index vs target, upload rate, plus **Scraper & workers (legacy-rollback-host)** — workers `up`, TT error rate, success/min, active syncs, GQL throttles.
